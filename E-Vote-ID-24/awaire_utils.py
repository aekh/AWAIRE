...
from irvballot_fast import Ballots, Ballot, perm_decode
import csv
...
class Requirement:
    ...
    """
    def __init__(self, audit: Audit, ident: ReqIdent, cvrs=None):
        self.audit = audit
        ...
        if cvrs is None:
            ...
            a, b, _ = self.audit.testmart.init_params()
            x = cvrs.to_bernoulli_asym(self.elim_fst, self.first, self.elim_snd, self.second)
            eta = np.mean(x)
            if eta <= 0.5:
                eta = 0.51
            self.params = (a, b, eta)
...
class RequirementHandler:
    def __init__(self, audit, parking, pruning_thold, pruning, history_size: int, verbose=0, cvrs=None):
        self.audit = audit
        ...
        # statistics
        self.stat_max_reqs = 0
        self.stat_tot_reqs = 0
        self.stat_sum_reqs = 0
        self.stat_max_req_parked = 0
        self.stat_sum_req_parked = 0
        self.stat_tot_req_prune = 0
    ...
    def process(self):
        ...
        # stats
        cur_reqs = len(self.reqs.values())
        n_parked = len(self.parked.keys())
        n_pruned = len(self.pruned)

        self.stat_max_reqs = max(cur_reqs, self.stat_max_reqs)
        self.stat_tot_reqs = cur_reqs + n_pruned + n_parked
        self.stat_sum_reqs += cur_reqs
        self.stat_sum_req_parked += n_parked
        self.stat_max_req_parked = max(n_parked, self.stat_max_req_parked)
        self.stat_tot_req_prune = n_pruned

        if self.verbose >= 1:
            print(f"Processing ballots, {cur_reqs} requirements active, {n_parked} requirements parked, {n_pruned} requirements pruned.")
        ...
        for req in self.reqs.values():
            ...
            if self.pruning >= 2 and result == -1:
                ...
                to_prune = req.ident.true_idents_if_false()
                if self.verbose >= 2:
                    print(f"Rejected! Inverses pruned:, {[i for i in to_prune]}.")
                self.prune_next += to_prune
            elif self.pruning >= 1 and result == 1:
                # Mathematically proven true: Prune self
                if self.verbose >= 2:
                    print(f"Proven true! Pruned:, {req.ident}.")
        ...
        if self.verbose >= 1:
            print(f" ==> Finished processing ballots, {len(self.reqs)} requirements active, {len(self.parked)} requirements parked, {len(self.pruned)} requirements pruned.")
    ...
    def get(self, ident: ReqIdent):
        ...
        if ident in self.parked.keys():
            ...
            if self.verbose >= 2:
                print(f"Unparked:, {ident}, behind {self.audit.ballots.nballots - self.reqs[ident].n_processed}.")
            # Catch up
            req = self.reqs[ident]
            result = req.process(self.history_size)  # TODO this should return running value?
            if self.pruning >= 2 and result == -1:
                ...
                to_prune = req.ident.true_idents_if_false()
                if self.verbose >= 2:
                    print(f"(Wakeup) Rejected! Inverses pruned:, {[i for i in to_prune]}.")
                self.prune_next += to_prune
            elif self.pruning >= 1 and result == 1:
                # Mathematically proven true: Prune self
                if self.verbose >= 2:
                    print(f"(Wakeup) Proven true! Pruned:, {req.ident}.")
                self.prune_next += [req.ident]
            return self.reqs[ident]
        elif ident in self.pruned:
            return None
        else:
            new_req = Requirement(self.audit, ident, cvrs=self.cvrs)
...
class Largest(Weigher):
    def compute(self, indices: np.ndarray, requirements: set):
        weights = []
        for i in indices-1:
            if i == 0:  # no samples taken yet, use assorter margin to inform weights
                etas = np.array([r.params[2] for r in requirements])
                weights.append(etas == max(etas))
            else:  # samples taken, use past to inform weights
...
class Frontier:
    def __init__(self, audit: Audit, weigher: Weigher, **kwargs):
        ...
        self.node_expand_condition = kwargs.get("node_expand_condition", -np.inf)
        self.node_expand_every = kwargs.get("node_expand_every", 25)  # TODO: make expansion rule more fluid? e.g. how many, which one etc.
        self.verbose = kwargs.get("verbose", 0)  # 1 = verbose, 2 = super verbose
        self.cvrs = kwargs.get("cvrs", None)

        self.reqs = RequirementHandler(audit, parking=self.req_parking, pruning_thold=self.req_pruning_thold,
                                       pruning=self.req_pruning, history_size=history_size, verbose=self.verbose,
                                       cvrs=self.cvrs)  # dict of active requirements
        ...
        # statistics
        self.stat_max_nodes = 0
        self.stat_tot_nodes = 0
        self.stat_sum_nodes = 0
        self.stat_tot_prune = 0
        self.stat_sum_prune_depth = 0
        self.stat_max_depth = 0
    ...
    def create_frontier(self):
        ...
        if self.node_full_start:
            for order in permutations(range(ncand)):
            ...
            for i in range(ncand):
                ...
                node.queue_counter = self.next_queue_count()
                self.nodes.put(node)
            self.stat_max_depth = 1
        self.stat_tot_nodes = self.nodes.qsize()
        if self.verbose >= 1:
            print(f"Initialised search tree with {len([n for n in self.nodes.queue])} nodes: {[n for n in self.nodes.queue]}")
        if self.verbose >= 1:
            print(f"Initialised requirement handler with {len([r for r in self.reqs.reqs])} requirements: {[r for r in self.reqs.reqs]}")
    ...
    def try_expand_node(self, node, processed_nodes, force=False):
        ...
        for child in node.expand():
            new_reqs[child] = [self.reqs.get(i) for i in child.request_requirements()]
            #for new_req in new_reqs:
        ...
        def get_score(x):
            ...
            else: return x.values[-1]

        if max(get_score(i) for i in sum(new_reqs.values(), [])) > max(self.node_expand_condition, node.score) or force:
            for child in new_reqs.keys():
                for new_req in new_reqs[child]:
                    child.add_to_watchlist(new_req)
                processed_nodes.append(child)
            self.stat_max_depth = max(self.stat_max_depth, len(node.elimination_suffix)+1)
            node.prune()
        else:
            return False
        return True
    ...
    def process_nodes(self):
        cur_nodes = self.nodes.qsize()
        processed_nodes = []
        budget = 0
        if self.n_processed_ballots > 0 and self.n_processed_ballots % self.node_expand_every == 0:
            budget += 1
        if self.verbose >= 1:
            print(f"Processing nodes, observed {self.n_processed_ballots} ballots, {self.nodes.qsize()} nodes in queue, {budget} budget")
        while not self.nodes.empty():
            node = self.nodes.get()
            if self.verbose >= 2:
                print(f" . Processing node: {node} with score {node.score}, and ISMG value {node.value} with running max {node.rmax}")

            # Auto-expand if only one child
            if len(node.elimination_suffix) == self.audit.contest.n_candidates - 1:
                if self.verbose >= 2:
                    print(" . . AUTO EXPAND, one child only")
                children = []
                self.try_expand_node(node, children, force=True)
                child = children[0]
                child.queue_counter = self.next_queue_count()
            ...
            if len(node.watchlist) == 0:
                ...
                if node.has_children():
                    self.try_expand_node(node, children, force=True)
                    if self.verbose >= 2:
                        print(" . . REQ EXPAND, no requirements watched")
                    cur_nodes = cur_nodes - 1 + len(children)
                    self.stat_tot_nodes += len(children)
            ...
            if node.value >= self.audit.threshold:
                if self.verbose >= 2:
                    print(f" . . PRUNE, new value over {node.value} over threshold")
                self.stat_tot_prune += 1
                self.stat_sum_prune_depth += len(node.elimination_suffix)
                node.prune()
                continue

            # Check if expand (via threshold)
            if node.base_max <= self.node_expand_threshold:
                x = len(processed_nodes)
                if self.try_expand_node(node, processed_nodes):
                    self.stat_tot_nodes += len(processed_nodes) - x
                    continue
        ...
        if self.verbose >= 1:
            print(f" ==> Finished processing nodes, {self.nodes.qsize()} nodes in queue, {budget} budget, queue counter: {node.queue_counter}.")

        #  0: continue,  1: certify,  -1: escalate
        action = 0
        if self.escalate:
            action = -1
        elif len(processed_nodes) == 0:
            action = 1

        # finalise stats
        self.stat_max_nodes = max(self.stat_max_nodes, cur_nodes)
        self.stat_sum_nodes += cur_nodes
