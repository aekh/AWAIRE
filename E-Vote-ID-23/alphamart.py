...
# Copyright (C) 2023 Philip B. Stark
#
# SHANGRLA is free software: you can redistribute it and/or modify it under the
...
#
# print(
# alpha_mart(np.concatenate((x,x)), N=200, eta=0.52), "\n",
# alpha_mart(x, N=200-len(x), eta=0.52, mu=0.49473684, estim=lambda x, N, mu, nu, f, u: shrink_trunc(x=x, N=N, mu=mu, nu=nu, f=f, u=u, S0=sum(x), j0=len(x)+1))
# )

# with open('outfile.csv', 'w') as sys.stdout:
#     print("zeroes, halfs, ones, mean, median, mode, std, skew, kurtosis, run, samplesize")
#
#     for pad in range(208):
#         x1 = [0]*(207-pad)
#         x2 = [0.5]*(0+2*pad)
#         x3 = [1]*(253-pad)
#         x = np.array(x1 + x2 + x3)
#         xmean = np.mean(x)
#         xmed = np.median(x)
#         xmode = sp.stats.mode(x, keepdims=False)[0]
#         xsd = np.std(x)
#         xskew = sp.stats.skew(x)
#         xkurt = sp.stats.kurtosis(x)
#         l1, l2, l3 = len(x1), len(x2), len(x3)
#         for r in range(100):
#             np.random.seed(r)
#             np.random.shuffle(x)
#             etas, mus = shrink_trunc(x, N=l1+l2+l3, nu=xmean)
#             terms, _ = alpha_mart(x, mus, etas)
#             samplesize = np.argwhere(terms>20)[0,0]
#             print(l1, l2, l3, xmean, xmed, xmode, xsd, xskew, xkurt, r, samplesize, sep=", ")
