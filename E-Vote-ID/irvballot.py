...
def perm_decode(code, length):
    ...
    for d in reversed(range(1, length+1)):
        if code == 0:
            lcode += [0]*d
            break
        # print(code, length, lcode, d, np.math.factorial(d-1), end=" ")
        div, code = np.divmod(code, np.math.factorial(d-1))
        # print(div)
        lcode += [div]
...
class Ballots(object):
    ...
    def simulate(self):
        """
        Simulates election. returns list of candidates in order of elimination
        """
        sequence = []
        while len(sequence) < self.ncand:
    ...
    def to_bernoulli(self, elims: list, a: int, b: int, length=None):
        ...
        if length is None:
            ...
            res = map(func, self.draworder[-25:])

        return list(map(func, self.draworder))
